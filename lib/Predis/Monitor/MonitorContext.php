 ...
 */
class Predis_Pipeline_MonitorContext implements \Iterator
{
    private $client;
    private $isValid;
    private $position;
    ...
    private function checkCapabilities(Predis_ClientInterface $client)
    {
        if ($client->getConnection() instanceof Predis_Connection_AggregatedConnectionInterface) {
            throw new Predis_NotSupportedException('Cannot initialize a monitor context when using aggregated connections');
        }
        if ($client->getProfile()->supportsCommand('monitor') === false) {
            throw new Predis_NotSupportedException('The current profile does not support the MONITOR command');
        }
    }
