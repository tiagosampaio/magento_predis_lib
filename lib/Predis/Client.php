 ...
 */
class Predis_Client implements Predis_ClientInterface
{
    const VERSION = '0.8.4';
    ...
    {
        if (!isset($options)) {
            return new Predis_Option_ClientOptions();
        }

        if (is_array($options)) {
            return new Predis_Option_ClientOptions($options);
        }

        if ($options instanceof Predis_Option_ClientOptionsInterface) {
            return $options;
        }
    ...
    {
        if ($parameters instanceof Predis_Connection_ConnectionInterface) {
            return $parameters;
        }
        ...
        if (is_callable($parameters)) {
            ...
            if (!$connection instanceof Predis_Connection_ConnectionInterface) {
    ...
    {
        if (!$this->connection instanceof Predis_Connection_AggregatedConnectionInterface) {
            throw new Predis_NotSupportedException('Retrieving connections by ID is supported only when using aggregated connections');
        }
    ...
    public function __call($method, $arguments)
    {
        ...
        if ($response instanceof Predis_ResponseObjectInterface) {
            if ($response instanceof Predis_ResponseErrorInterface) {
     ...
     * @return mixed
     */
    protected function onResponseError(Predis_Command_CommandInterface $command, Predis_ResponseErrorInterface $response)
    {
        ...
        if ($this->options->exceptions) {
            throw new Predis_ServerException($response->getMessage());
        }
    ...
    protected function initPipeline(Array $options = null, $callable = null)
    {
        ...
        $pipeline = new Predis_Pipeline_PipelineContext($this, $executor);
     ...
     * @return PipelineContext|array
     */
    private function pipelineExecute(Predis_Pipeline_PipelineContext $pipeline, $callable)
    {
    ...
    {
        $transaction = new Predis_Transaction_MultiExecContext($this, $options ?: array());
    ...
    {
        $pubsub = new Predis_PubSub_PubSubContext($this, $options);
    ...
    {
        return new Predis_Pipeline_MonitorContext($this);
    }
}
