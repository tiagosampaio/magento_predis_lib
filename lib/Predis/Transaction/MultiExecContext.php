 ...
 */
class Predis_Transaction_MultiExecContext implements Predis_BasicClientInterface, Predis_ExecutableContextInterface
{
     ...
     */
    public function __construct(Predis_ClientInterface $client, Array $options = null)
    {
     ...
     * @param ClientInterface $client Client instance used by the context.
     */
    private function checkCapabilities(Predis_ClientInterface $client)
    {
        if ($client->getConnection() instanceof Predis_Connection_AggregatedConnectionInterface) {
            throw new Predis_NotSupportedException('Cannot initialize a MULTI/EXEC context when using aggregated connections');
        }

        $profile = $client->getProfile();

        if ($profile->supportsCommands(array('multi', 'exec', 'discard')) === false) {
            throw new Predis_NotSupportedException('The current profile does not support MULTI, EXEC and DISCARD');
        }
    ...
    private function isWatchSupported()
    {
        if ($this->canWatch === false) {
            throw new Predis_NotSupportedException('The current profile does not support WATCH and UNWATCH');
        }
    }
    ...
    public function watch($keys)
    {
        ...
        if ($this->checkState(self::STATE_INITIALIZED) && !$this->checkState(self::STATE_CAS)) {
            throw new Predis_ClientException('WATCH after MULTI is not allowed');
        }
    ...
    private function checkBeforeExecution($callable)
    {
        if ($this->checkState(self::STATE_INSIDEBLOCK)) {
            throw new Predis_ClientException("Cannot invoke 'execute' or 'exec' inside an active client transaction block");
        }
        ...
        if ($callable) {
            ...
            if (!$this->commands->isEmpty()) {
                $this->discard();
                throw new Predis_ClientException('Cannot execute a transaction block after using fluent interface');
            }
        }
    ...
    public function execute($callable = null)
    {
        ...
        do {
            ...
            if ($reply === null) {
                if ($attempts === 0) {
                    ...
                    throw new Predis_Transaction_AbortedMultiExecException($this, $message);
                }
        ...
        for ($i = 0; $i < $size; $i++) {
            ...
            if ($commandReply instanceof ResponseErrorInterface && $useExceptions) {
                ...
                throw new Predis_ServerException($message);
            }
