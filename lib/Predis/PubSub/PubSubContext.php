 ...
 */
class Predis_PubSub_PubSubContext extends AbstractPubSubContext
{
    private $client;
    private $options;
    ...
    private function checkCapabilities(Predis_ClientInterface $client)
    {
        if ($client->getConnection() instanceof Predis_Connection_AggregatedConnectionInterface) {
            throw new Predis_NotSupportedException('Cannot initialize a PUB/SUB context when using aggregated connections');
        }

        $commands = array('publish', 'subscribe', 'unsubscribe', 'psubscribe', 'punsubscribe');

        if ($client->getProfile()->supportsCommands($commands) === false) {
            throw new Predis_NotSupportedException('The current profile does not support PUB/SUB related commands');
        }
    }
    ...
    protected function getValue()
    {
        ...
        switch ($response[0]) {
            ...
            default:
                ...
                throw new Predis_ClientException($message);
        }
    }
}
