 ...
 */
class Predis_Pipeline_MultiExecExecutor implements Predis_Pipeline_PipelineExecutorInterface
{
    protected $profile;
     ...
     */
    protected function checkConnection(Predis_Connection_ConnectionInterface $connection)
    {
        if (!$connection instanceof SingleConnectionInterface) {
            $class = __CLASS__;
            throw new Predis_ClientException("$class can be used only with single connections");
        }
    }
     ...
     * {@inheritdoc}
     */
    public function execute(Predis_Connection_ConnectionInterface $connection, SplQueue $commands)
    {
        ...
        if (!isset($responses)) {
            throw new Predis_ClientException('The underlying transaction has been aborted by the server');
        }

        if (count($responses) !== count($commands)) {
            throw new Predis_ClientException("Invalid number of replies [expected: ".count($commands)." - actual: ".count($responses)."]");
        }
    ...
    public function setProfile(ServerProfileInterface $profile)
    {
        if (!$profile->supportsCommands(array('multi', 'exec', 'discard'))) {
            throw new Predis_ClientException('The specified server profile must support MULTI, EXEC and DISCARD.');
        }
