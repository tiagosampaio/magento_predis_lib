 ...
 */
class Predis_Option_ClientReplication extends Predis_Option_AbstractOption
{
    /**
    ...
    public function filter(Predis_Option_ClientOptionsInterface $options, $value)
    {
        ...
        if (is_string($value)) {
            ...
            if (!($connection = new $value()) instanceof Predis_Connection_ReplicationConnectionInterface) {
                throw new \InvalidArgumentException('Instance of Predis\Connection\ReplicationConnectionInterface expected');
            }
    ...
    {
        return new Predis_Connection_MasterSlaveReplication();
    }
}
