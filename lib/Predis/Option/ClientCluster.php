 ...
 */
class Predis_Option_ClientCluster extends Predis_Option_AbstractOption
{
    /**
    ...
    {
        if (!$cluster instanceof Predis_Option_ClusterConnectionInterface) {
     ...
     * @return \Closure
     */
    protected function getInitializer(Predis_Option_ClientOptionsInterface $options, $fqnOrType)
    {
        switch ($fqnOrType) {
            case 'predis':
                return function () {
                    return new Predis_Connection_PredisCluster();
                };

            case 'redis':
                return function () use ($options) {
                    $connectionFactory = $options->connections;
                    $cluster = new Predis_Connection_RedisCluster($connectionFactory);
