 ...
 */
class Predis_Connection_RedisCluster implements Predis_Connection_ClusterConnectionInterface, \IteratorAggregate, \Countable
{
    private $pool;
    private $slots;
    private $slotsMap;
     ...
     */
    public function __construct(Predis_Connection_ConnectionFactoryInterface $connections = null)
    {
        $this->pool = array();
        $this->slots = array();
        $this->strategy = new Predis_Cluster_RedisClusterHashStrategy();
        $this->connections = $connections ?: new Predis_Connection_ConnectionFactory();
    }
     ...
     * {@inheritdoc}
     */
    public function getConnection(Predis_Command_CommandInterface $command)
    {
        $hash = $this->strategy->getHash($command);

        if (!isset($hash)) {
            throw new Predis_NotSupportedException("Cannot use {$command->getId()} with redis-cluster");
        }
     ...
     * @return mixed
     */
    protected function onMoveRequest(Predis_Command_CommandInterface $command, $request, $details)
    {
        ...
        switch ($request) {
            ...
            default:
                throw new Predis_ClientException("Unexpected request type for a move request: $request");
        }
    }
     ...
     * @return mixed
     */
    protected function handleServerError(Predis_Command_CommandInterface $command, ResponseErrorInterface $error)
    {
