 ...
 */
class Predis_Connection_PredisCluster implements Predis_Connection_ClusterConnectionInterface, \IteratorAggregate, \Countable
{
    private $pool;
    private $strategy;
    private $distributor;
    ...
    {
        $distributor = $distributor ?: new Predis_Cluster_Distribution_HashRing();

        $this->pool = array();
        $this->strategy = new Predis_Cluster_PredisClusterHashStrategy($distributor->getHashGenerator());
    ...
    public function getConnection(Predis_Command_CommandInterface $command)
    {
        ...
        if (!isset($hash)) {
            throw new Predis_NotSupportedException("Cannot use {$command->getId()} with a cluster of connections");
        }
     ...
     * @return array
     */
    public function executeCommandOnNodes(Predis_Command_CommandInterface $command)
    {
        $replies = array();
