 ...
 */
class Predis_Connection_WebdisConnection implements Predis_Connection_SingleConnectionInterface
{
    ...
    {
        $class = __CLASS__;
        throw new Predis_NotSupportedException("The method $class::$function() is not supported");
    }
    ...
    private function checkExtensions()
    {
        if (!function_exists('curl_init')) {
            throw new Predis_NotSupportedException(sprintf(self::ERR_MSG_EXTENSION, 'curl'));
        }

        if (!function_exists('phpiredis_reader_create')) {
            throw new Predis_NotSupportedException(sprintf(self::ERR_MSG_EXTENSION, 'phpiredis'));
        }
    }
     ...
     * @return resource
     */
    private function initializeCurl(Predis_Connection_ConnectionParametersInterface $parameters)
    {
        $options = array(
     ...
     * @return resource
     */
    private function initializeReader(Predis_Connection_ConnectionParametersInterface $parameters)
    {
     ...
     * @return string
     */
    protected function getCommandId(Predis_Command_CommandInterface $command)
    {
        switch (($commandId = $command->getId())) {
            ...
            case 'DISCARD':
            case 'MONITOR':
                throw new Predis_NotSupportedException("Disabled command: {$command->getId()}");
     ...
     * {@inheritdoc}
     */
    public function writeCommand(Predis_Command_CommandInterface $command)
    {
    ...
    public function executeCommand(Predis_Command_CommandInterface $command)
    {
        ...
        if (curl_exec($resource) === false) {
            ...
            throw new Predis_Connection_ConnectionException($this, trim($error), $errno);
        }

        if (phpiredis_reader_get_state($this->reader) !== PHPIREDIS_READER_STATE_COMPLETE) {
            throw new Predis_Protocol_ProtocolException($this, phpiredis_reader_get_error($this->reader));
        }
